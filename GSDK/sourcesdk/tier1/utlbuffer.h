...
void CByteswap::SwapFieldToTargetEndian(void* pOutputBuffer, void* pData, typedescription_t* pField)
{
	...
	case FIELD_EMBEDDED:
	{
		...
		{
			SwapFieldsToTargetEndian((byte*)pOutputBuffer + pEmbed->fieldOffset[TD_OFFSET_NORMAL], (byte*)pData + pEmbed->fieldOffset[TD_OFFSET_NORMAL], pField->td);
...
public:
	...
	//void			Swap(CUtlMemory<BYTE> &mem);

	void ActivateByteSwappingIfBigEndian(void)
	{
		if (false)
	...
	void			Printf(PRINTF_FORMAT_STRING const char* pFmt, ...);
