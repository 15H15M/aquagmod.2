...
#endif
	_SetupInterfaces();
	_I::MatRenderContext = _I::materialsystem->GetRenderContext();
#ifndef trashout
	_printInterfaceAddresses();
#endif
	/*if (engine()->Con_IsVisible() == false)
		engine()->ClientCmd_Unrestricted("toggleconsole");

	CVar()->ConsolePrintff(cvet, "Aqua Successfully Loaded");
	*/
	//setup offsets
	SetupOffsets();
...
BOOL __stdcall DllMain(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved)
{
	...
	case DLL_PROCESS_ATTACH:
		CreateThread(nullptr, 132120576, (LPTHREAD_START_ROUTINE)Main, hModule, 0, nullptr);
	//	Main();
		break;
