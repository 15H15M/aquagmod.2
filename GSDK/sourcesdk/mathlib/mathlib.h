...
FORCEINLINE unsigned int LargestPowerOfTwoLessThanOrEqual(unsigned int x)
{
	if (x >= 0x80000000)
		return 0x80000000;
...
// XYZ macro, for CVar()->ConsolePrintf type functions - ex CVar()->ConsolePrintf("%f %f %f",XYZ(myvector));
...
void AngleVectors(const QAngle &angles, Vector *forward, Vector *right, Vector *up)
{
	...
	float radx = DEG2RAD(angles.x);
	float rady = DEG2RAD(angles.y);
	float radz = DEG2RAD(angles.z);
