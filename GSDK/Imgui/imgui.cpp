...
#include <stdio.h>      // vsnprintf, sscanf, CVar()->ConsolePrintf
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags)
{
	...
	window->DrawList->AddRectFilledMultiColor(frame_bb.Min + ImVec2(3.5f, 3.f), frame_bb.Max + ImVec2(-2.5f, -3.f), GetColorU32(ImVec4(0.13f, 0.13f, 0.13f, 1.f)), GetColorU32(ImVec4(0.13f, 0.13f, 0.13f, 1.f)), GetColorU32(ImVec4(0.07f, 0.07f, 0.07f, 1.f)), GetColorU32(ImVec4(0.07f, 0.07f, 0.07f, 1.f)));
	...
	window->DrawList->AddRectFilledMultiColor(frame_bb.Min + ImVec2(7.f, 6.f), bb_slider.Max + ImVec2(-5.f, -6.f), GetColorU32(ImGuiCol_MainTheme), GetColorU32(ImGuiCol_MainTheme2), GetColorU32(ImGuiCol_MainTheme), GetColorU32(ImGuiCol_MainTheme2));
	PopClipRect();
...
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* display_format, float power)
{
	...
	RenderTextClipped(frame_bb.Min, frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(0.5f, 1.5f));
		...
		struct Funcs
		{
			static void NodeDrawList(ImDrawList* draw_list, const char* label)
			{
				...
				for (const ImDrawCmd* pcmd = draw_list->CmdBuffer.begin(); pcmd < draw_list->CmdBuffer.end(); elem_offset += pcmd->ElemCount, pcmd++)
				{
					...
					if (!pcmd_node_open)
						continue;
					ImGuiListClipper clipper(pcmd->ElemCount / 3); // Manually coarse clip our CVar()->ConsolePrintf out of individual vertices to save CPU, only items that may be visible.
					while (clipper.Step())
