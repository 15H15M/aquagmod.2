...
void bf_write::WriteVarInt64(UINT64 data)
{
	...
	if ((m_iCurBit & 7) == 0 && (m_iCurBit + bitbuf::kMaxVarintBytes * 8) <= m_nDataBits)
	{
		...
		// processors.
		UINT64 part0 = static_cast<UINT32>(data);
		UINT64 part1 = static_cast<UINT32>(data >> 28);
		UINT64 part2 = static_cast<UINT32>(data >> 56);
...
int bf_read::ReadBitsClamped_ptr(void *pOutData, size_t outSizeBytes, size_t nBits)
{
	...
	{
		// Should we CVar()->ConsolePrintf a message when we clamp the data being read? Only
...
#ifdef BIT_VERBOSE
	Con_CVar()->ConsolePrintf("PeekBitLong:  %i %i\n", nShifts, (unsigned int)r);
#endif
