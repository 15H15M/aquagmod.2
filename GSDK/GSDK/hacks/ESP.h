...
namespace H
{
	...
	{
#ifndef trashout
		void DrawSomeExtraInfo(BaseEntity* Ent)
		{
			//if (boolka == 1 && !strcmp(Ent->GetLuaScriptName(), "vendor_npc"))
			//{
			//	boolka = 0;
			//	//Ent->~BaseEntity();
			//}
			...
			if (HackVars::Visuals::TargetObsInfo)
			{
				//if (Ent->IsNPC())
				{
					H::Draw::Drawtext(Ent->GetLuaName(), HackVars::Fonts::DefaultFont, x + w + 2, y + iY, Color(255 / 2, 255, 0, 255));
					iY += 15;

					H::Draw::Drawtext(Ent->GetLuaScriptName(), HackVars::Fonts::DefaultFont, x + w + 2, y + iY, Color(255 / 2, 255, 0, 255));
					iY += 15;
					//char ww[10];
					//sCVar()->ConsolePrintff(ww,"%d",Ent->);
					//H::Draw::Drawtext(ww, HackVars::Fonts::DefaultFont, x + w + 2, y + iY, Color(255 / 2, 255, 0, 255));
					//iY += 15;
				}
				/*C_BasePlayer* localplayer = (C_BasePlayer*)cliententitylist()->GetClientEntity(engine()->GetLocalPlayer());
				Ent->GetOwner*/
				/*BaseEntity* benty = localplayer->GetObserverTarget();
				if (benty)
				{
					CVar()->ConsoleDCVar()->ConsolePrintff(localplayer->GetObserverTarget()->GetLuaName());
					CVar()->ConsoleDCVar()->ConsolePrintff(localplayer->GetObserverTarget()->GetLuaScriptName());
					H::Draw::Drawtext(localplayer->GetObserverTarget()->GetLuaName(), 1, x + w + 2, y + iY, Color(255 / 2, 255, 0, 255));
					iY += 15;

					H::Draw::Drawtext(localplayer->GetObserverTarget()->GetLuaScriptName(), 1, x + w + 2, y + iY, Color(255 / 2, 255, 0, 255));
					iY += 15;
				}*/
			}
		}
